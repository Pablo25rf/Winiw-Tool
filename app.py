        ...
        if df_exec.empty:
            ...
            if not df_chart.empty:
                ...
                _bars = (alt.Chart(df_chart)
                    ...
                    .encode(
                        y=alt.Y('centro:N', sort=_sort_order,
                                axis=alt.Axis(labelColor='white', labelFontSize=13,
                                              labelFontWeight='bold', titleColor='white',
                                              tickColor='white', title=None)),
                        x=alt.X('score_medio:Q', scale=alt.Scale(domain=[0, _x_max]),
    ...
    with tab_dsp:
        ...
        try:
            ...
            if df_ss.empty:
                ...
                df_latest_ss = (df_ss.sort_values('timestamp', ascending=False)
                ...
                if not df_latest_ss.empty:
                    ...
                    _y_max_ss = min(115, float(df_latest_ss['score_f'].max()) + 25)
                    ...
                    _text_ss = (alt.Chart(df_latest_ss)
                        .mark_text(dy=-8, fontSize=13, fontWeight='bold', color='white')
                        .encode(x=alt.X('centro:N', sort=_sort_ss),
                                y=alt.Y('score_f:Q', scale=alt.Scale(domain=[_y_min_ss, _y_max_ss])),
                                text=alt.Text('label:N')))
                    st.altair_chart((_bars_ss + _text_ss).properties(padding={"top": 20}),
                                    use_container_width=True)
                ...
                df_tr_ss = df_tr_ss.dropna(subset=['score_f']).sort_values('timestamp')
                if len(df_tr_ss) < 2:
                    st.info("Se necesitan al menos 2 semanas de datos para mostrar la tendencia.")
                else:
